...
export function Modal({ note, setModalComponent,modalComponent }: modalProps) {
  return (
    <div className={`flex justify-center items-center fixed h-screen w-full top-0 ${modalComponent? "":"hidden"}`}>
      <div onClick={()=>{setModalComponent(false)}} className="w-full fixed top-0 left-0 backdrop-blur-[3px] backdrop-brightness-90 h-full"></div>
      <div className="divide-solid divide-y dark:divide-slate-900 shadow-2xl bg-gray-50 dark:bg-slate-800 px-8 py-6 z-10 rounded-lg w-[50rem]">
        ...
        <div className="break-words whitespace-pre-line focus:outline-none w-full dark:bg-slate-700 bg-white px-4 py-2 rounded lg:mt-10 mt-6 dark:text-gray-100">
        {note.content}
        </div>
      </div>
    </div>
